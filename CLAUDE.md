...
**2. Does `R` replace `S` or supplement it?** The addendum's read `v̂ = c + q^T R` has no `S` term, and the companion flowchart's State box holds only `C` and `R` — both imply `R` *replaces* `S`. Say so explicitly, because the capacity accounting differs sharply: replacement costs only the codebook plus the query→cluster carrier over baseline, whereas supplementing doubles the state outright. Either way, `S`/`R` are activations, not weights, so principle #3's matched-parameter control does not constrain state size at all — **add a matched-state control** (Gated DeltaNet with the equivalent total state) and report against both. A capacity claim measured against a baseline holding less state proves nothing.
